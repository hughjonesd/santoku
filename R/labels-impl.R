...
endpoint_labels.Date <- function (breaks, raw, fmt = NULL, ...) {
  endpoint_labels_datetime(
    breaks, raw, fmt, class = "Date", default_fmt = "%F",
...
endpoint_labels.POSIXt <- function (breaks, raw, fmt = NULL, ...) {
  endpoint_labels_datetime(
    breaks, raw, fmt, class = "POSIXt", default_fmt = "%F %H:%M:%S",
    origin = as.POSIXct("1970-01-01")
  )
}


endpoint_labels_datetime <- function(
    breaks, raw, fmt, class, default_fmt, origin
) {
  endpoints <- scaled_endpoints(breaks, raw = raw)
  if (!inherits(endpoints, class)) {
    return(if (is.null(fmt)) base::format(endpoints) else apply_format(fmt, endpoints))
  }

  labels <- apply_format(fmt %||% default_fmt, endpoints)
  labels[is.infinite(endpoints) & endpoints < origin] <- symbol_infinity(minus = TRUE)
  labels[is.infinite(endpoints) & endpoints > origin] <- symbol_infinity()
  labels
}
...
scaled_endpoints <- function (breaks, raw) {
  endpoints <- if (is.breaks(breaks)) unclass_breaks(breaks) else breaks
  if (raw) endpoints else attr(breaks, "scaled_endpoints") %||% endpoints
}
...
apply_format <- function (fmt, endpoint, ...) {
  if (is.function(fmt)) return(fmt(endpoint, ...))
  if (is.list(fmt)) return(do.call(base::format, c(list(x = endpoint), fmt)))
  if (is.numeric(endpoint) || is.character(endpoint)) {
    return(sprintf(fmt, endpoint, ...))
  }
...
#' Apply optional templates to singleton, first, and last interval labels
#'
#' @noRd
apply_interval_overrides <- function(
    labels, pieces, l, r, single = NULL, first = NULL, last = NULL
) {
  render <- function(template, i) {
    glue::glue(
      template,
      l = l[i],
      r = r[i],
      l_closed = pieces$l_closed[i],
      r_closed = pieces$r_closed[i]
    )
  }

  if (!is.null(single)) {
    labels[pieces$singletons] <- render(single, pieces$singletons)
  }
  if (!is.null(first)) labels[1] <- render(first, 1)
  if (!is.null(last)) labels[length(labels)] <- render(last, length(labels))
  labels
}
