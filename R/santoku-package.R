...
#' the function [kut()], which is similar to base R's [cut()] or `Hmisc::cut2()`.
#' `kut(x, breaks)` takes a numeric vector `x` and returns a factor of the
...
#'
#' * Unlike `cut()` or `cut2()`, `kut()` can handle single values as well as
#' intervals. For example, `kut(x, breaks = c(1, 2, 2, 3))` will create a
...
#'
#' * Convenience functions for quickly tabulating cut data.
#'
