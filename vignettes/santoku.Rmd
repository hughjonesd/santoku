...
```{r, eval = requireNamespace("lubridate", quietly = TRUE)}
data.frame(
  y2k = y2k,
  chopped = chop_width(y2k, lubridate::months(1))
)
```
...
data.frame(
  y2k = y2k,
  chopped = chop_width(y2k, lubridate::months(1), labels = lbl_discrete(fmt = "%e %b"))
)
```

You can also chop vectors with units, using the `units` package:

```{r, eval = requireNamespace("units", quietly = TRUE)}
library(units)
