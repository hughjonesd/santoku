...
```{r}
demo_chop(brk_quantiles(c(0.05, 0.25, 0.5, 0.75, 0.95)))
```
...
```{r}
demo_chop(brk_width(200), labels = lbl_LETTERS(), cex_labels = 1)
```
...
```{r}
demo_chop(brk_width(200), labels = lbl_roman("(%s)"), cex_labels = 1)
```
