...
```{r}
demo_chop(brk_width(width = 200), close_end = TRUE)
```
...
```{r}
demo_chop(brk_evenly(intervals = 5), close_end = TRUE)
```
...
```{r}
demo_chop(brk_n(100), close_end = TRUE)
```
...
```{r}
demo_chop(brk_quantiles(c(0.05, 0.25, 0.5, 0.75, 0.95)), close_end = TRUE)
```
