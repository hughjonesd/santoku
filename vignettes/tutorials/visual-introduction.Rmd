...
demo_kut <- function (breaks, labels = lbl_intervals(), extend = NULL, ...,
      lines = TRUE, swoosh = FALSE, lty = 1, cex_labels = 0.7,
      skip_lines = numeric(0)) {
  kut_vec <- kut(pts$x, breaks, labels, extend, ...)
  ...
  kut_cols <- palette()[kut_vec]
  kut_cols[is.na(kut_vec)] <- NA_col
  plot_the_fish(cols = kut_cols)
  ...
  if (lines) {
    ...
    if (swoosh) {
      ...
      cols <- palette()[seq_len(nlevels(kut_vec))]
...
`kut()` is a replacement for base R's `cut()` function.
...
```{r}
demo_kut(c(300, 600, 900))
```

## `extend = FALSE`

```{r, echo = TRUE, eval = FALSE}
kut(x, c(300, 600, 900), extend = FALSE)
```

```{r}
demo_kut(c(300, 600, 900), extend = FALSE)
legend("bottomright", "NA", col = NA_col, pch = 19, bty = "n")
```

## Cutting by a single value


```{r, echo = TRUE, eval = FALSE}
kut(x, c(300, 500, 500, 800))
```

```{r}
demo_kut(c(300, 500, 500, 800), skip_lines = 500)
...
Cuts fixed-width intervals

```{r, echo = TRUE, eval = FALSE}
kut_width(x, width = 200)
```

```{r}
demo_kut(brk_width(width = 200))
```

## `kut_evenly()`

Cuts `groups` equal-width intervals


```{r, echo = TRUE, eval = FALSE}
kut_evenly(x, groups = 5)
```

```{r}
demo_kut(brk_evenly(groups = 5))
```

## `kut_equally()`

Cuts intervals with an equal *number of elements*

```{r, echo = TRUE, eval = FALSE}
kut_equally(x, groups = 5)
```

```{r}
demo_kut(brk_quantiles(1:4/5))
```

## `kut_n()`

Cuts intervals with a *fixed* number of elements
...
Cut by:            | number of elements | width
--------------------|--------------------|-------------
Fixed size          | `kut_n()`         | `kut_width()`
Fixed no. of groups | `kut_equally()`   | `kut_evenly()`

## `kut_quantiles()`


```{r, echo = TRUE, eval = FALSE}
kut_quantiles(x, c(0.05, 0.25, 0.5, 0.75, 0.95)
```

```{r}
demo_kut(brk_quantiles(c(0.05, 0.25, 0.5, 0.75, 0.95)))
```
...
```{r, echo = TRUE, eval = FALSE}
kut(x, c(300, 600, 900), labels = LETTERS[1:4])
```

```{r}
demo_kut(c(300, 600, 900), labels = LETTERS[1:4], cex_labels = 1)
```
...
```{r, echo = TRUE, eval = FALSE}
kut_width(x, 200, labels = lbl_LETTERS())
```


```{r}
demo_kut(brk_width(200), labels = lbl_LETTERS(), cex_labels = 1)
```
...
```{r, echo = TRUE, eval = FALSE}
kut_width(x, 200, labels = lbl_roman("(%s)"))
```


```{r}
demo_kut(brk_width(200), labels = lbl_roman("(%s)"), cex_labels = 1)
```
...
```{r, echo = TRUE, eval = FALSE}
kut_width(x, 200, labels = lbl_dash())
```


```{r}
demo_kut(brk_width(200), labels = lbl_dash(), cex_labels = 0.8)
```
...
```{r}
demo_kut(c(200, 500, 800), swoosh = TRUE)
```
...
```{r, echo = TRUE, eval = FALSE}
kut(x, brk_right(c(200, 500, 800)))
```

```{r}
demo_kut(brk_right(c(200, 500, 800)), swoosh = TRUE)
```

## Errors

![Cutting fail](https://media.giphy.com/media/2zom27Xqp7PVe/giphy.gif)
...
```{r, echo = TRUE}
kut_quantiles(c(-Inf, Inf), c(0.25, 0.75))
```
...
![Chopping bottles](https://media.giphy.com/media/ceQ6oQNVCVbgY/giphy.gif)